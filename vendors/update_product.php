 ...
 if($file_extension != 'jpg' && $file_extension != 'png' && $file_extension != 'jpeg' &&
    ...
    mysqli_query($con,"set names 'utf8'");
		...
		if($file_name!=''){
			...
			$query3 = "UPDATE `products` SET `name`='$pName',`name_ar`='$pNameAr',`description`='$pDesc',`description_ar`='$pDescAr',`price`='$pPrice',`product_type_id`='$pTypeId',`delivery_type_id`='$pDelId' WHERE id = '$pid'";
            ...
            <div  class="secBox">
                ...
                <select style="height: auto" name="pdelivery">
                    ...
                    while($row1 = $result1 -> fetch_array()){
						?>
                            <option value="<?php echo $row1['id'];?>" <?php if($row1['id']==$row2['delivery_type_id']){echo "selected";}?> ><?php echo $row1['types'] ?></option>;
                        <?php
                    }
                    ?>
                </select>
